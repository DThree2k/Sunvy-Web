...
const MainChat = (props) => {
  ...
  useEffect(() => {
    socketState.on("broadcast_to_all_user_in_room", (data) => {
      setListMessage((prev) => [...prev, {...data, conversation: prev[0]?.conversation}]);
    });
  }, [socketState]); 
    ...
    <div className={"jdahdjksdhjashasjfda"} style={{ flex: "1 1 0" }}>
      ...
      >
        <TitleMainChat
          ...
          {...props?.conversation}
          avtChat={props?.conversation?.imageGroup || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'}
...
const TitleMainChat = memo((props) => {
  ...
  let imageUrl = props?.imageGroup ? props?.imageGroup : props?.member?.filter(item => item?._id !== Cookies.get("uid"))?.[0]?.profilePicture 
